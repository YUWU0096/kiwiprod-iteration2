...
if __name__ == '__main__':
    ...
    pd.DataFrame(constants.heap_index_values.keys(), columns=['english_profeciency']) \
        .to_csv("english_profeciency_index.csv", index=True, index_label='id')

    # load the definition of OCCP from abs.gov.au
    # load from abs.gov.au
    # occp_anzsco_df = pd.read_excel("https://www.abs.gov.au/ausstats/subscriber.nsf/log?openagent&1220.0%20ANZSCO%20Version%201.2%20Structure%20v3.xls&1220.0"
    #                                "&Data%20Cubes&6A8A6C9AC322D9ABCA257B9E0011956C&0&2013,%20Version%201.2&05.07.2013&Latest",
    #                                sheet_name="Table 6").drop(list(range(0, 6, 1372)), axis=0).dropna().reset_index(drop=True)
    ...
    # au_micro = pd.read_csv("https://www.abs.gov.au/AUSSTATS/subscriber.nsf/log?openagent&acmid16_testfile.csv&3417.0.55.001&Data%20Cubes"
    #                        "&7157AA0BF63DA7FECA2584D5001C5AFB&0&2016&20.12.2019&Latest", dtype=object)
    ...
    au_micro = au_micro[(au_micro['highest_education'].notnull()) &
                        (au_micro['highest_education'] != "Unknown education background")]

    # convert employment status to employed or not
    ...
    au_micro = au_micro[au_micro.OCCP.notna()]
    au_micro = au_micro[au_micro.OCCP != 'XXXXXX']
    au_micro['occupation'] = au_micro.OCCP.apply(lambda x: int(x)).replace(occp_anzsco_dict)
    ...
    ml_model = SVC(kernel="linear", C=0.025, probability=True)
    ml_model.fit(au_micro.loc[:, au_micro.columns != 'occupation'], au_micro.occupation)

    # export model in ONNX
    ml_onnx = convert_sklearn(ml_model, initial_types=[('float_input', FloatTensorType([None, 4]))])
